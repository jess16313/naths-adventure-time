...
export default function Kidnapper({ currentGmId }) {
  ...
  const fetchKidnapperData = async () => {
    ...
    if (data) {
      ...
      // Locate kidnapper profile records dynamically
      const self = data.find(p => p.role?.toLowerCase() === 'kidnapper');
      if (self) {
  ...
  // ⚡ FIXED REALTIME EFFECT: Subscribes cleanly ONCE on mount
  useEffect(() => {
    fetchKidnapperData();

    // Creates a single, dedicated subscription channel
    const channel = supabase
      .channel('public_player_changes')
      .on(
        'postgres_changes', 
        { 
          event: '*', // Catch updates, inserts, deletions
          schema: 'public', 
          table: 'player' 
        }, 
        (payload) => {
          console.log('Realtime game state payload received:', payload);
          fetchKidnapperData(); // Trigger full refresh safely
        }
      )
      .subscribe((status) => {
        console.log("Kidnapper websocket connectivity status:", status);
      });
  ...
  const handleStartInterview = async (player) => {
    ...
    try {
      ...
      // 2. Safely grab the kidnapper row using role matching
      const self = players.find(p => p.role?.toLowerCase() === 'kidnapper');
      ...
      if (self) {
        ...
        alert(`Success! Capturing ${player.character_name}.`);
      }
    } catch (err) {
      alert("Error processing interview initiation: " + err.message);
    }
  };
  ...
  const handleEndInterview = async () => {
    ...
    try {
      ...
      if (self) {
        ...
        alert(`${activeTarget.character_name} released back to base.`);
      }
    } catch (err) {
      alert("Error releasing player: " + err.message);
    }
  };
  ...
  const handleVerifyAccusation = () => {
    ...
    const trueThievesIds = players
      .filter(p => p.role?.toLowerCase() === 'thief')
      .map(p => p.id);
    ...
    <div className="space-y-6 text-gray-200 animate-fadeIn">
      ...
      {activeTab === 'interviews' && (
        <div className="space-y-4">
          {activeTarget ? (
            <div className="bg-red-950/40 border border-red-500/30 rounded-2xl p-5 text-center space-y-3">
          ...
          {!activeTarget && (
            <div className="space-y-2">
              ...
              {players.map((p) => {
                ...
                if (roleLower === 'kidnapper' || roleLower === 'hint giver') return null;
      ...
      {/* 🔵 TAB 2: ACCUSATIONS INTERFACE MODE */}
