...
export default function App() {
  ...
  // Hook 1: Persistent storage authentication link check
  useEffect(() => {
  ...
  // Hook 2: Initialize OneSignal Global SDK on Mount
  useEffect(() => {
    OneSignal.init({
      appId: "97d83ae0-54b0-4bdf-9ced-69ceab157324",
      allowLocalhostAsSecureOrigin: true
    }).then(() => {
      console.log("OneSignal SDK successfully loaded.");
    }).catch((err) => {
      console.error("OneSignal load fail:", err);
    });
  }, []);

  // Hook 3: Tag OneSignal User whenever character changes
  useEffect(() => {
    if (character && character.role) {
      // Tags the device in OneSignal dashboard so you can target "host" or others
      OneSignal.User.addTag("role", character.role.toLowerCase());
    }
  }, [character]);

  // Hook 4: Real-time system broadcast stream
  useEffect(() => {
    if (character) {
  ...
  // Hook 5: Global breakthrough and elimination watcher
  useEffect(() => {
    if (character) {
      ...
      <div className="flex flex-col items-center justify-center h-screen bg-slate-950 p-6 text-white text-center">
        <div className="max-w-md w-full space-y-6 bg-slate-900 p-8 rounded-2xl border border-slate-800 shadow-2xl">
          ...
          <form onSubmit={handleLogin} className="space-y-4">
            ...
            <button type="submit" className="w-full bg-red-600 hover:bg-red-700 text-white font-bold py-3 rounded-lg tracking-wide uppercase transition mt-2 text-xs">
              Decrypt Case Files 🔓
            </button>
          </form>
        </div>
            ...
            <div className="bg-slate-900 p-5 rounded-xl border border-slate-800">
              ...
              <div className="mt-4 pt-4 border-t border-slate-800">
                <span className={`px-3 py-1 rounded-full text-xs font-bold uppercase tracking-wider ${
                  { murderer: 'bg-red-600 text-white', accomplice: 'bg-orange-600 text-white', imposter: 'bg-yellow-600 text-white', bystander: 'bg-green-600 text-white', medium: 'bg-purple-600 text-white' }[character.role] || 'bg-slate-800 text-slate-300'
                }`}>
                  Role: {character.role}
                </span>
              </div>
